...
<script runat="server">
    private UserInfo GetUser()
    {
        ...
        int userID;
        if (!Int32.TryParse(HttpContext.Current.User.Identity.Name, out userID)) return null;

        var userService = DependencyResolver.Current.GetService<IUserService>();
        if (userService == null) return null;

        var user = userService.GetItemByUserID(userID, UserStatus.Normal);
        if (user == null) throw new NullReferenceException("用户未通过认证");
    ...
    private int GetUserID()
    {
        ...
        int userID;
        if (!Int32.TryParse(HttpContext.Current.User.Identity.Name, out userID)) return 0;

        var userService = DependencyResolver.Current.GetService<IUserService>();
        if (userService == null) return 0;

        // TODO: 下行不可行
        //if (!userService.IsExistsAsync(userID, UserStatus.Normal).Result) return 0;
        if (!userService.IsExists(userID, UserStatus.Normal)) return 0;
    ...
    public override void SetConfig()
    {
        ...
        // Panorama:/UserResources/企业ID/Panoramas/PanoramaID
        // Room:    /UserResources/企业ID/Rooms/RoomID
        // Weixin:  /UserResources/企业ID/Weixins/WeixinID
        // 不同的类型，图片大小、比例等都作相应约束。
        var userID = GetUserID();
        // The base URL used to reach files in CKFinder through the browser.
        BaseUrl = "/UserResources/" + userID + "/";
        ...
        type = ResourceType.Add("Head");
        type.Url = BaseUrl + "head/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "head/";
        type.MaxSize = 200 * 1024;
        type.AllowedExtensions = new string[] { "jpeg", "jpg", "png" };
        type.DeniedExtensions = new string[] { };

        type = ResourceType.Add("Logo");
        type.Url = BaseUrl + "logo/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "logo/";
        type.MaxSize = 200 * 1024;
        type.AllowedExtensions = new string[] { "jpeg", "jpg", "png" };
        type.DeniedExtensions = new string[] { };

        type = ResourceType.Add("ProductIcon");
        type.Url = "/Products/Icons/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "logo/";
        type.MaxSize = 200 * 1024;
        type.AllowedExtensions = new string[] { "jpeg", "jpg", "png" };
        type.DeniedExtensions = new string[] { };

        type = ResourceType.Add("ExpressProductIcon");
        type.Url = "/ExpressProduct/Icons/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "logo/";
        type.MaxSize = 200 * 1024;
        type.AllowedExtensions = new string[] { "jpeg", "jpg", "png" };
        type.DeniedExtensions = new string[] { };
